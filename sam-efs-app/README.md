...
```bash
sam-efs-app$ sam logs -n HelloEfsFunction --stack-name sam-efs-app --tail
```
...
```bash
sam-efs-app$ pip install pytest pytest-mock --user
sam-efs-app$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name sam-efs-app
```
